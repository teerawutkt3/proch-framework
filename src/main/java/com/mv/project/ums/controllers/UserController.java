...
public class UserController {
    ...
    public ResponseData<?> changePassword(@RequestBody UserVo userVo) {
        ...
        try {
            userSerivce.changePassowrd(userVo);
            MessageUtil.setMessageSuccess(responseData, ProjectConstant.ResponseMessage.Save.SUCCESS);
            logger.info("Method changePassword process success {}", userVo.getUser().toString());
        } catch (Exception e) {
            e.printStackTrace();
            MessageUtil.setMessageFail(responseData, ProjectConstant.ResponseMessage.Save.FAILED);
            logger.error("Method changePassword process fail {}", userVo.getUser().toString());
        }
        return responseData;
    }
}